...
public class Service
{
	...
	public void menu(sbyte typeclose, int npcId, int menuId, int optionId)
	{
		Message message = null;
		try
		{
			message = new Message((sbyte)29);
			message.writer().writeByte(typeclose);
	...
	public void sendGPS()
	{
		...
		try
		{
			message = messageNotMap(-59);
			message.writer().writeUTF(GameMidlet.longitude);
			message.writer().writeUTF(GameMidlet.latitude);
			session.sendMessage(message);
		}
		catch (Exception)
		{
	...
	public void doRemoveVithu(int id)
	{
		Message message = null;
		try
		{
			message = new Message((sbyte)117);

...
public class GameCanvas : IActionListener
{
	...
	{
		//Debug.Log("Mã lỗi  : " + s + "- Loại lỗi : " + type);
	}
	...
	{
		g.drawRegion(y0: ((!noRotate) ? (gameTick % 3) : 0) * 16, arg0: imgShuriken, x0: 0, w0: 16, h0: 16, arg5: 0, x: x, y: y, arg8: mGraphics.HCENTER | mGraphics.VCENTER);
	}
	...
	public void perform(int idAction, object p)
	{
		...
		case 8885:
			...
			break;
		case 8886:
			startYesNoDlg(mResources.NEW_ACC_ARLET, new Command(mResources.COUNTINUE, instance, 8880, null), new Command(mResources.NO, instance, 8882, null));
			break;
		case 8880:
			endDlg();
		...
		case 88817:
			...
			{
				Service.gI().menu(0, Char.getMyChar().npcFocus.template.npcTemplateId, menu.menuSelectedItem, 0);
			}
			else
			{
				Service.gI().menu(0, 0, menu.menuSelectedItem, 0);
			}
			break;
		case 888181:
		{
		...
		case 88820:
		{
			...
			else
			{
				...
				Service.gI().menu(0, Char.getMyChar().npcFocus.template.npcTemplateId, menuSelectedItem, 0);
			}
			break;
		}
		case 88821:
		{
			int menuId = (int)p;
			ChatPopup.addChatPopup(string.Empty, 1, Char.getMyChar().npcFocus);
			Service.gI().menu(0, Char.getMyChar().npcFocus.template.npcTemplateId, menuId, menu.menuSelectedItem);
			break;
		}
		case 88822:
			ChatPopup.addChatPopup(string.Empty, 1, Char.getMyChar().npcFocus);

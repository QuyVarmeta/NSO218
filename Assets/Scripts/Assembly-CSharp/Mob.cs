...
public class Mob : MainObject
{
	...
	public static string MobName(int templateId)
	{
		for (int i = 0; i < GameScr.vMob.size(); i++)
		{
			...
			{
				return mob.getTemplate().name;
			}
		}
		return "";
	}
	public static bool MobContain(int templateId)
	{
		for (int i = 0; i < GameScr.vMob.size(); i++)
		{
			Mob mob = (Mob)GameScr.vMob.elementAt(i);
	...
	private void updateTypeMobAttk()
	{
		if (p1 == 0)
		{
			...
			num2 = cFocus.cy;
			typeAtt = typeAtk;
			if (arrMobTemplate[templateId].speed > 0)
			{
				dir = ((x < num) ? 1 : 0);
			}
			if (isBoss)
	...
	public void update_oldMob(int line)
	{
	...
	public void update_New_MOB(int line)
	{
		if (owner == null)
		{
			return;
		...
		sbyte speed = arrMobTemplate[templateId].speed;
		if (owner.statusMe != 1 && owner.statusMe != 6 && getDistance(x, y, owner.cx, owner.cy) > arrMobTemplate[templateId].rangeMove)
		{
			b = (sbyte)(arrMobTemplate[templateId].speed + 2);
		}
		b += (sbyte)(mobId % 2);
		speed += (sbyte)(mobId % 2);
		if (owner.statusMe == 1 || owner.statusMe == 6)
		{
			if (x - xFirst >= arrMobTemplate[templateId].rangeMove)
			{
				isright = true;
			...
			else if (xFirst - x >= arrMobTemplate[templateId].rangeMove)
			{
				isleft = true;
				isright = false;
			}
	...
	public void updateMobWalk()
	{
		...
		try
		{
			...
			case 3:
			{
				...
				else if (GameCanvas.gameTick % 2 == 1 && arrMobTemplate[templateId].speed != 0)
				{
					b--;
				}
				x += b * dir;
	...
	{
		if ((arrMobTemplate[templateid].type == 4 || arrMobTemplate[templateid].type == 5) && status == 3)
		{
			isNotRangeMove = true;
	...
	public Image loadImageMob(int i)
	{
		return GameCanvas.loadImage("/" + templateId + "/" + (i + 1) + ".png");
	}
	...
	{
		if (arrMobTemplate[templateId].frameBossMove != null)
		{
			p1++;
			if (GameCanvas.gameTick % 2 == 0)
			{
			...
			if (frameIndex >= arrMobTemplate[templateId].frameBossMove.Length)
			{
				frameIndex = 0;
			}
			frame = arrMobTemplate[templateId].frameBossMove[frameIndex];
	...
	{
		if (arrMobTemplate[templateId].frameBossAttack == null)
		{
			return;
		}
		if (p1 == 0)
	...
	public void paintBottomMobNew(mGraphics g)
	{
		try
		{
			if (GameCanvas.gameTick % 2 == 0 && status != 3)
			{
				tt++;
				if (tt > getTemplate().frameBossMove.Length - 1)
				{
					tt = 0;
				}
				this.frame = getTemplate().frameBossMove[tt];
			}
			Frame frame = getTemplate().frameBoss[this.frame];
			for (int i = 0; i < frame.dx.Length; i++)
			{
				...
				int num5 = frame.dx[i];
				int num6 = frame.dy[i];
				if (num > getTemplate().imgs[0].getWidth())
				{
					num = 0;
				}
				if (num2 > getTemplate().imgs[0].getHeight())
				{
					num2 = 0;
				}
				if (num + num3 > getTemplate().imgs[0].getWidth())
				{
					num3 = getTemplate().imgs[0].getWidth() - num;
				}
				if (num2 + num4 > getTemplate().imgs[0].getHeight())
				{
					num4 = getTemplate().imgs[0].getHeight() - num2;
				}
				int num7 = ((frame.flip[i] == 1) ? 2 : 0);
				if (dir == 1)
				{
					if (num7 == 2)
					{
				...
				g.drawRegion(getTemplate().imgs[0], num, num2, num3, num4, num7, x + num5, y + num6, 0);
			}
		}
		catch (Exception)
		{
	...
	public void paintTopMobNew(mGraphics g)
	{
		try
		{
			Frame frame = getTemplate().frameBoss[this.frame];
	...
	public void paintBoTomMobNew(mGraphics g, int x, int y)
	{
		try
		{
			if (GameCanvas.gameTick % 2 == 0 && status != 3)
	...
	public void paintTopMobNew(mGraphics g, int x, int y)
	{
		try
		{
			Frame frame = getTemplate().frameBoss[this.frame];
	...
	public int Absx()
	{
		return Math.abs(xFirst - Char.getMyChar().cx);
	}

	public int Absx(Char s)
	{
		return Math.abs(xFirst - s.cx);
	}

	public int Absy()
	{
		return Math.abs(yFirst - Char.getMyChar().cy);
	}

	public int Absy(Char s)
	{
		return Math.abs(yFirst - s.cy);
	}
}

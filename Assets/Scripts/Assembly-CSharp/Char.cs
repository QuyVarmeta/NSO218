...
public class Char : MainObject
{
	...
	public static mFont[] color_font_name = new mFont[4]
	{
		mFont.tahoma_7_white,
		mFont.tahoma_7_blue1,
		mFont.tahoma_7b_purple,
	...
	public static bool b(int paramInt1, int paramInt2)
	{
		if (paramInt1 == -1)
		{
			paramInt1 = myChar.cx;
		...
		int[] array = new int[2];
		if (TileMap.a(paramInt1, paramInt2, array))
		{
			return Move(array[0], array[1]);
		}
		return false;
	}
	public static int pFidnNpc(int paramInt1, int paramInt2, int paramInt3, int paramInt4)
	{
		if ((paramInt1 = (paramInt1 - paramInt3) * (paramInt1 - paramInt3) + (paramInt2 - paramInt4) * (paramInt2 - paramInt4)) <= 0)
		{
			return 0;
		}
		paramInt2 = (paramInt1 + 1) / 2;
		do
		{
			paramInt3 = paramInt2;
			paramInt2 = paramInt2 / 2 + paramInt1 / (paramInt2 << 1);
		}
		while (Math.abs(paramInt3 - paramInt2) > 1);
		return paramInt2;
	}
	public static void PickNpc(int npcID, int menuID, int optionID)
	{
		if (mSystem.currentTimeMillis() < 500)
		{
			Code.Sleep(500 - (int)mSystem.currentTimeMillis());
		}
		Npc npc;
		if ((npc = FindNpc(npcID)) != null)
		{
			Move(npc.cx, npc.cy);
			getMyChar().npcFocus = npc;
			Service.gI().menu(0,npcID, menuID, optionID);
		}
	}
	public static Npc FindNpc(int var0)
	{
		Char @char = getMyChar();
		MyVector vNpc = GameScr.vNpc;
		int num = -1;
		Npc result = null;
		for (int i = 0; i < vNpc.size(); i++)
		{
			Npc npc;
			if ((npc = (Npc)vNpc.elementAt(i)) != null && var0 == npc.template.npcTemplateId)
			{
				int num2 = pFidnNpc(@char.cx, @char.cy, npc.cx, npc.cy);
				if (num == -1 || num2 < num)
				{
					num = num2;
					result = npc;
	...
	public static TaskOrder FindTask(int taskId)
	{
		for (int i = 0; i < getMyChar().taskOrders.size(); i++)
		{
			TaskOrder taskOrder;
			if ((taskOrder = (TaskOrder)getMyChar().taskOrders.elementAt(i)) != null && taskOrder.taskId == taskId)
			{
				return taskOrder;
			}
		}
	...
	public static bool Move(int cxSend, int cySend)
	{
		Char @char = getMyChar();
		if (cxSend == -1)
		{
		...
		if (cxSend == @char.cx && cySend == @char.cy)
		{
			return false;
		}
		int num = @char.cx;
		int n = (TileMap.tileTypeAt(cxSend, cySend - 12, 64) ? (TileMap.tileYofPixel(cySend) - 24) : cySend);
		if (cxSend > num)
		{
			do
			{
				Service.gI().charMove("-11",num, TileMap.pMove(num, n));
			}
			while ((num += 50) < cxSend);
		}
		else
		...
		Service.gI().charMove("-11", cxSend, cySend);
		@char.cx = (@char.cxSend = cxSend);
		@char.cy = (@char.cySend = cySend);
		return true;
	}
	...
	public void sendMove()
	{
		if (me && cHP > 0)
		{
			...
			if ((distance > 0 || distance2 > 0) && mSystem.currentTimeMillis() - timeSendmove >= 250 || Code.isHackGiay)
			{
				isSendMove = true;
			}
			if (isSendMove)
	...
	public void updateFramecharByhorse()
	{
		...
		DataSkillEff partEff = getPartEff(ID_HORSE, true);
		if (partEff != null && partEff.isLoadData)
		{
			...
			dynewhhorse = partEff.dyHorse + partEff.Dy_Char[getFrameHores()];
			dxnewhorse = partEff.dxHorse + partEff.Dx_Char[getFrameHores()];
			if (cdir == 1)
			{
				dxnewhorse *= -1;
			}
			...
			if (GameCanvas.gameTick % 3 == 0)
			{
				...
				FramecharRideHorse = partEff.FrameChar[getFrameHores()];
			}
	...
	public void updateFrameSuSan()
	{
		...
		DataSkillEff partEff = getPartEff(ID_SUSANO, false);
		if (partEff != null && partEff.sequence != null)
		{
			fsusan++;
			if (fsusan > partEff.sequence.Length - 1)
			{
				fsusan = 0;
			}
			FrameSusan = partEff.sequence[fsusan];
		}
	}
	...
	public void updateFrameRank()
	{
		...
		DataSkillEff partEff = getPartEff(ID_RANK, false);
		if (partEff != null && partEff.sequence != null)
		{
			fRank++;
	...
	public void updateFrameName()
	{
		...
		DataSkillEff partEff = getPartEff(ID_NAME, false);
		if (partEff != null && partEff.sequence != null)
		{
			fName++;
	...
	public virtual void update()
	{
		...
		if (me && isAHP && Code.T == null && cHP < cMaxHP * aHpValue / 100 && mSystem.currentTimeMillis() - lastUseHP > 300 && statusMe != 14 && statusMe != 5 && cHP > 0)
		{
			if (vEff.size() == 0)
			{
		...
		if (me && isAFood && Code.T == null && GameCanvas.gameTick % 10 == 0 && !GameScr.isPaintAuto && statusMe != 14 && statusMe != 5 && cHP > 0)
		{
			if (vEff.size() == 0)
			{
		...
		if (me && isABuff && Code.T == null && getMyChar().statusMe != 14 && getMyChar().statusMe != 5 && cHP > 0 && mSystem.currentTimeMillis() - lastTimeUseSkill > 500)
		{
		...
		if (me && isAMP && Code.T == null && cMP < cMaxMP * aMpValue / 100 && GameCanvas.gameTick % 4 == 0 && statusMe != 14 && statusMe != 5 && cHP > 0)
		{
			GameScr.gI().doUseMP();
		}
		...
		else
		{
			if (GameCanvas.gameTick % 20 == 0 && charID >= 0)
			{
				for (int num8 = 0; num8 < GameScr.vCharInMap.size(); num8++)
				{
					...
					if (char2 == null || char2.Equals(this))
					{
					}
				}
			}
	...
	public void updateNew_MobVithu()
	{
		...
		if (mobVithu.getTemplate() != null && mobVithu.getTemplate().typeFly == 1)
		{
			if (Mob.getDistance(mobVithu.x, mobVithu.y, cx, cy) > Mob.arrMobTemplate[mobVithu.templateId].rangeMove + 100)
			{
				mobVithu.xFirst = cx + (3 - GameCanvas.gameTick % 6) * 6;
				mobVithu.yFirst = cy - 60;
				int num = mobVithu.xFirst - mobVithu.x;
				int num2 = mobVithu.yFirst - mobVithu.y;
				if (num > 50 || num < -50)
				{
					mobVithu.x += num / 10;
				}
		...
		if (mobVithu.status != 3)
		{
			if (cdir == -1)
			{
				mobVithu.xFirst = cx + 20;
				mobVithu.yFirst = cy;
				mobVithu.dir = cdir;
				mobVithu.y = cy - 20;
			}
			...
			int num3 = mobVithu.xFirst - mobVithu.x;
			int num4 = mobVithu.yFirst - mobVithu.y;
			if (num3 > 50 || num3 < -50)
			{
				mobVithu.x += num3 / 10;
			}
		...
		try
		{
			...
			if (me)
			{
				...
				if (skillPaint != null && (mobFocus != null || (charFocus != null && isMeCanAttackOtherPlayer(charFocus))))
				{
					...
					if (effPaints.Length > 1)
					{
						...
						Lightning.addLight(myVector3, firstPos, true, getClassColor());
					}
			...
			else if (skillPaint != null && (mobFocus != null || charFocus != null))
			{
				...
				if (effPaints.Length > 1 && effPaints[0] != null)
				{
					...
					num = 9;
					Lightning.addLight(myVector4, firstPos2, true, getClassColor());
				}
	...
	public void paintCharName_HP_MP_Overhead(mGraphics g)
	{
		...
		if (me)
		{
			...
			else if (ColorName > 0)
			{
				...
				{
					color_font_name[ColorName].drawString(g, cName, cx, cy - heightCharName, mFont.CENTER, mFont.tahoma_7_grey);
				}
				heightCharName++;
			}
		...
		{
			heightCharName += color_font_name[ColorName].getHeight();
			color_font_name[ColorName].drawString(g, mResources.BY + " " + getMyChar().cName + " " + mResources.PROTECT, cx, cy - heightCharName, mFont.CENTER, mFont.tahoma_7_grey);
			heightCharName++;
		}
		...
		else if (charID >= 0)
		{
			...
			else if (cTypePk == PK_PHE4)
			{
				heightCharName += SmallImage.smallImg[1241][4];
				SmallImage.drawSmallImage(g, 3410, cx, cy - heightCharName, 0, mGraphics.HCENTER | mGraphics.TOP);
				heightCharName++;
			}
	...
	public void updatefHorseui()
	{
		if (GameCanvas.gameTick % 3 == 0)
		{
			...
			{
				fhorsetrangbi = (fhorsetrangbi + 1) % partEff.ActionStand.Length;
			}
		}
	}

	public void paintHorseUI(mGraphics g, int x, int y)
	{
		DataSkillEff partEff = getPartEff(ID_HORSE, true);
		if (partEff == null || !partEff.isLoadData)
		{
		...
		partEff.paintBottomEff_new(g, x, y, getFrameHorseui(partEff.ActionStand[fhorsetrangbi]), 2);
		partEff.paintTopEff_new(g, x, y, getFrameHorseui(partEff.ActionStand[fhorsetrangbi]), 2);
	}
	...
	public void paintMatna(mGraphics g, int cx, int cy, int f)
	{
		...
		DataSkillEff partEff = getPartEff(ID_MAT_NA, false);
		int num = 0;
	...
	{
		DataSkillEff partEff = getPartEff(ID_LEG, false);
		int num = 0;
		if ((statusMe == 1 || statusMe == 6) && cf <= 1)
		{
			num = cf * f;
	...
	{
		DataSkillEff partEff = getPartEff(ID_PP, false);
		if (partEff != null)
		{
			partEff.paintBottomEff_new(g, cx, cy, getFramePP(f), 0);
		}
	}
	...
	{
		DataSkillEff partEff = getPartEff(ID_WEA_PONE, false);
		if (partEff == null)
		{
			return;
		}
	...
	{
		DataSkillEff partEff = getPartEff(ID_HAIR, false);
		if (partEff == null)
		{
			return;
		}
	...
	{
		DataSkillEff partEff = getPartEff(ID_Body, false);
		if (partEff == null)
		{
			return;
		}
	...
	public int getDyHorse()
	{
		if (ID_HORSE > -1)
		{
			...
			if (partEff != null && partEff.isLoadData)
			{
				return partEff.dyHorse + partEff.Dy_Char[getFrameHores()];
			}
		}
	...
	private void paintCharWithoutSkill(mGraphics g)
	{
		...
		{
			DataSkillEff partEff = getPartEff(ID_WEA_PONE, false);
			if (partEff != null && GameCanvas.gameTick % 3 == 0)
			{
				int num = partEff.listFrame.size() / 30;
				if (num == 0)
				{
			...
			DataSkillEff partEff2 = getPartEff(ID_Bien_Hinh, false);
			...
			DataSkillEff partEff3 = getPartEff(ID_MAT_NA, false);
			...
			DataSkillEff partEff4 = getPartEff(ID_SUSANO, false);
			DataSkillEff partEff5 = getPartEff(ID_RANK, false);
			DataSkillEff partEff6 = getPartEff(ID_NAME, false);
			DataSkillEff partEff7 = getPartEff(ID_LEG, false);
			if (partEff7 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num4 = partEff7.listFrame.size() / 30;
				if (num4 == 0)
				{
					num4 = 1;
				}
			...
			DataSkillEff partEff8 = getPartEff(ID_HORSE, true);
			if (partEff8 != null && partEff8.isLoadData && GameCanvas.gameTick % 10 == 0)
			{
				int num5 = partEff8.listFrame.size() / partEff8.sequence.Length;
				if (num5 == 0)
				{
					num5 = 1;
				}
			...
			DataSkillEff partEff9 = getPartEff(ID_Body, false);
			if (partEff9 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num6 = partEff9.listFrame.size() / 30;
				if (num6 == 0)
				{
					num6 = 1;
				}
			...
			DataSkillEff partEff10 = getPartEff(ID_HAIR, false);
			if (partEff10 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num7 = partEff10.listFrame.size() / 30;
				if (num7 == 0)
				{
					num7 = 1;
				}
			...
			DataSkillEff partEff11 = getPartEff(ID_PP, false);
			if (partEff11 != null && GameCanvas.gameTick % 6 == 0)
			{
				int num8 = partEff11.listFrame.size() / 30;
				if (num8 == 0)
				{
					num8 = 1;
				}
			...
			else
			{
				if (partEff4 != null)
				{
					partEff4.paintBottomEff_new(g, cx, cy - getDyHorse(), FrameSusan, (cdir != 1) ? 2 : 0);
				}
				if (partEff != null)
				{
				...
				if (partEff8 != null)
				{
					partEff8.paintBottomEff_new(g, cx, cy, getFrameHores(), (cdir == 1) ? 2 : 0);
				}
				if (partEff5 != null)
				{
					partEff5.paintBottomEff_new(g, cx, cy - getDyHorse(), FrameRank, (cdir != 1) ? 2 : 0);
				}
				if (partEff6 != null)
				{
					partEff6.paintBottomEff_new(g, cx, cy - getDyHorse(), FrameName, 0);
				}
				paintPP_Bot(g);
				if (isMoto)
				{
					...
					if (arrItemMounts[4].template.id == 485)
					{
						if (arrItemMounts[4].sys < 2)
						{
							if (statusMe == 1 || statusMe == 6)
							{
								...
								if ((partEff11 != null || partEff9 != null || partEff10 != null || partEff != null || partEff3 != null) && cf <= 1)
								{
									num9 = cf;
								}
							}
				...
				else
				{
					...
					if (isNewMount)
					{
						...
						if (partEff4 != null)
						{
							partEff4.paintTopEff_new(g, cx, cy - getDyHorse(), FrameSusan, (cdir != 1) ? 2 : 0);
						}
						paintPP_Top(g);
						if (partEff6 != null)
						{
							partEff6.paintTopEff_new(g, cx, cy - getDyHorse(), FrameName, 0);
						}
						if (partEff5 != null)
						{
							partEff5.paintTopEff_new(g, cx, cy - getDyHorse(), FrameRank, (cdir != 1) ? 2 : 0);
						}
						if (partEff != null)
						{
					...
					if (partEff8 == null)
					{
						if (partEff7 != null)
						{
							partEff7.paintBottomEff_new(g, cx, cy, getFrameLeg(), (cdir != 1) ? 2 : 0);
							partEff7.paintTopEff_new(g, cx, cy, getFrameLeg(), (cdir != 1) ? 2 : 0);
						}
						else
						{
					...
					if (statusMe != 2)
					{
						paintClanEffect(g, cx + 7 * cdir, cy - 2 - getDyHorse());
					}
					if (statusMe != 1 && statusMe != 6)
					{
						if (partEff10 != null)
						{
							partEff10.paintBottomEff_new(g, cx + dxnewhorse, cy - dynewhhorse, getFrameHair(), (cdir != 1) ? 2 : 0);
							partEff10.paintTopEff_new(g, cx + dxnewhorse, cy - dynewhhorse, getFrameHair(), (cdir != 1) ? 2 : 0);
						}
						else
						{
					...
					if (partEff9 != null)
					{
						partEff9.paintBottomEff_new(g, cx + dxnewhorse, cy - dynewhhorse, getFrameBody(), (cdir != 1) ? 2 : 0);
						partEff9.paintTopEff_new(g, cx + dxnewhorse, cy - dynewhhorse, getFrameBody(), (cdir != 1) ? 2 : 0);
					}
					else
					{
					...
					if (statusMe == 2)
					{
						paintClanEffectRun(g, cx - 14 * cdir, cy - 2 - getDyHorse());
						paintClanEffect2(g, cx + 7 * cdir, cy - 2 - getDyHorse());
					}
					else
					{
						paintClanEffect(g, cx - 7 * cdir, cy - 2 - getDyHorse());
						paintClanEffect2(g, cx + 11 * cdir, cy - 2 - getDyHorse());
					}
				}
				if (partEff8 != null)
				{
					partEff8.paintTopEff_new(g, cx, cy, getFrameHores(), (cdir == 1) ? 2 : 0);
				}
				if (partEff4 != null)
				{
					partEff4.paintTopEff_new(g, cx, cy - getDyHorse(), FrameSusan, (cdir != 1) ? 2 : 0);
			...
			else
			{
				for (int j = 0; j < GameScr.vCharInMap.size(); j++)
				{
					...
					else if (myChar.nClass.classId == 6)
					{
						...
						else if ((myChar.cTypePk != PK_PHE1 || char2.cTypePk == PK_PHE2 || char2.cTypePk == PK_PHE3 || char2.cTypePk == PK_PHE4) && (myChar.cTypePk != PK_PHE2 || char2.cTypePk == PK_PHE1 || char2.cTypePk == PK_PHE3 || char2.cTypePk == PK_PHE4) && (myChar.cTypePk != PK_PHE3 || char2.cTypePk == PK_PHE1 || char2.cTypePk == PK_PHE2 || char2.cTypePk == PK_PHE4) && (myChar.cTypePk != PK_PHE4 || char2.cTypePk == PK_PHE1 || char2.cTypePk == PK_PHE2 || char2.cTypePk == PK_PHE3) && char2.statusMe != 14 && char2.statusMe != 5)
						{
							focus.addElement(char2);
							if (charFocus != null && char2.Equals(charFocus))
							{
	...
	public bool isMeCanAttackOtherPlayer(Char cAtt)
	{
		...
		return ((((getMyChar().cTypePk == PK_PHE4 && (cAtt.cTypePk == PK_PHE3 || cAtt.cTypePk == PK_PHE1 || cAtt.cTypePk == PK_PHE2)) || (getMyChar().cTypePk == PK_PHE3 && (cAtt.cTypePk == PK_PHE4 || cAtt.cTypePk == PK_PHE1 || cAtt.cTypePk == PK_PHE2)) || (getMyChar().cTypePk == PK_PHE1 && (cAtt.cTypePk == PK_PHE4 || cAtt.cTypePk == PK_PHE2 || cAtt.cTypePk == PK_PHE3)) || (getMyChar().cTypePk == PK_PHE2 && (cAtt.cTypePk == PK_PHE4 || cAtt.cTypePk == PK_PHE1 || cAtt.cTypePk == PK_PHE3))) && !getMyChar().isTeam(cAtt) && !isLang()) || (cAtt.cTypePk == 3 && !getMyChar().isTeam(cAtt) && !isLang()) || (getMyChar().cTypePk == 3 && !getMyChar().isTeam(cAtt) && !isLang()) || (getMyChar().cTypePk == PK_NHOM && cAtt.cTypePk == PK_NHOM && !getMyChar().isTeam(cAtt) && !isLang()) || (getMyChar().testCharId >= 0 && getMyChar().testCharId == cAtt.charID) || (getMyChar().killCharId >= 0 && getMyChar().killCharId == cAtt.charID && !isLang()) || (cAtt.killCharId >= 0 && cAtt.killCharId == getMyChar().charID && !isLang())) && cAtt.statusMe != 14 && cAtt.statusMe != 5;
	}
	...
	{
		if (ID_HORSE > -1 || !isNewMount || frameMount == null)
		{
			return;
		}
		tickNewMount++;

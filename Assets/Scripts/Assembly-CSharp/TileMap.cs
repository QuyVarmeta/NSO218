...
public class TileMap
{
	...
	public static int[] totalTile = new int[4] { 120, 141, 143, 103 };
	...
	public static void FixMove(int n, int n2)
	{
		...
		{
			num3 = n2 - 60;
			Service.gI().charMove("--1", Char.getMyChar().cx, num3);
			num2 = 3;
		}
		for (int i = 0; i < num; i++)
		{
			cx = ((n > Char.getMyChar().cx) ? (cx + 50) : (cx - 50));
			Service.gI().charMove("--1", cx, num3);
		}
		Service.gI().charMove("--1", n, num3);
		for (int j = 0; j < num2; j++)
		{
			num3 = ((n2 > Char.getMyChar().cy) ? (num3 + 10) : (num3 - 10));
			Service.gI().charMove("--1",n, num3);
		}
		Service.gI().charMove("--1", n, n2);
		...
		try
		{
			...
			for (int l = 1; l < myVector4.size(); l++)
			{
				...
				else if ((npc = (Npc)GameScr.vNpc.elementAt(0)) != null && npc.statusMe != 15)
				{
					...
					Service.gI().menu(0,npc.template.npcTemplateId, 0, 0);
	...
	public static void loadMap(int tileId)
	{
		...
		try
		{
			...
			if (!GameCanvas.lowGraphic && !Main.isIpod && mGraphics.zoomLevel != 1)
			{
				...
				{
					Effect2.vAnimateEffect.addElement(new AnimateEffect(1, true, 10, 200));
				}
				if ((mapID >= 39 && mapID <= 44) || (mapID >= 46 && mapID <= 48) || mapID == 56 || (mapID >= 62 && mapID <= 65))
				{
					Effect2.vAnimateEffect.addElement(new AnimateEffect(3, true, Res.random(150, 200), 200));
				}
			}
			setPosWater();
		}
